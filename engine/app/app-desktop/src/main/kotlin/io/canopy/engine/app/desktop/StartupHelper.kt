 ...
 * - Linux (NVIDIA GPUs only): Spawns a child JVM process with the `__GL_THREADED_OPTIMIZATIONS` [Environment Variable]
 * [System.getenv] set to `0` (if it was not already). This is required for LWJGL3 to work on Linux with NVIDIA GPUs.
 *
...
object StartupHelper {
     ...
     */
    fun isLinuxNvidia(): Boolean = File("/proc/driver")
        .list {
                _,
                path: String,
            ->
     ...
     *
     * All [Environment Variables][System.getenv] are copied to the child JVM process (if it is spawned), as specified
     * by [ProcessBuilder.environment]; the same applies for [System Properties][System.getProperties].
     *
     * **Usage:**
     *
     * ```
