...
class SceneManager(private var physicsStep: Float = 1f / 60f, private val block: SceneManager.() -> Unit = {}) :
    ...
    fun tick(delta: Float) {
        ...
        if (isPhysicsFrame(delta)) {
            systems[TreeSystem.UpdatePhase.PhysicsPre]?.forEach { it.tick(physicsStep) }
            root.nodePhysicsUpdate(physicsStep)
            systems[TreeSystem.UpdatePhase.PhysicsPost]?.forEach { it.tick(physicsStep) }
        }

        systems[TreeSystem.UpdatePhase.FramePre]?.forEach { it.tick(delta) }
        root.nodeUpdate(delta)
        systems[TreeSystem.UpdatePhase.FramePost]?.forEach { it.tick(delta) }
    }
